...
def createEnergysystem():
    ...
    energysystem.add(
        components.Converter(
            ...
            outputs={
                bus_electricity: flows.Flow(
                    nominal_capacity=oemof.solph.Investment(
                        ep_costs=600/12), #oemof.tools.economics.annuity(200, 20, 0.05, u=None, cost_decrease=0)),
                    variable_costs=0.01
                ),
                bus_heat: flows.Flow()
            },
        ...
        components.Converter(
            ...
            outputs={
                bus_gas: flows.Flow(
                    nominal_capacity=oemof.solph.Investment(
                        ep_costs=60/12,
                        maximum =10,), #oemof.tools.economics.annuity(200, 20, 0.05, u=None, cost_decrease=0)),
                    variable_costs=0,
    ...
    # # create storage object representing a battery for testing circularity
    nominal_capacity = oemof.solph.Investment(
        ep_costs=1/12,
        maximum=100)#+oemof.tools.economics.annuity(300, 20, 0.05, u=None, cost_decrease=0))
    ...
    nominal_capacity = oemof.solph.Investment(
        ep_costs=10/12)#+oemof.tools.economics.annuity(300, 20, 0.05, u=None, cost_decrease=0))
    ...
    # # create storage object representing a heatstorage
    nominal_capacity = oemof.solph.Investment(
        ep_costs=0.1/12)#+oemof.tools.economics.annuity(300, 20, 0.05, u=None, cost_decrease=0))
...
def optimizeEnergysytem(energysystem):
    ##########################################################################
...
def main():
    """Build and optimise the energy system, propagate costs and plot/export."""
    energysystem = createEnergysystem()
    results, result_dict = optimizeEnergysytem(energysystem)

    all_flow_dict = calculate_costs_of_all_flows(results)

    # Convert tuple keys to "source->target" strings for JSON export.
    all_flow_dict_str_keys = {
        f"{f[0].label}->{f[1].label}": value
        for f, value in all_flow_dict.items()
    }
    ...
    if storage_content is not None:
        storage_content.to_json("storage.json", orient="records", indent=4)

    with open("flows.json", "w") as f:
        json.dump(all_flow_dict_str_keys, f, indent=4, default=str)
