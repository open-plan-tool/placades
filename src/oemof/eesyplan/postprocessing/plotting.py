...
from oemof.eesyplan.postprocessing.cost_calculation import (
    _sum_columns_by_level,
)


def plot_all_flows_plotly(
    all_flow_dict, storage_content=None, normalize=False, specific=True
):
    fig = make_subplots(
        rows=5,
        cols=1,
    ...
    for flow_name, flow_data in all_flow_dict.items():
        ...
        if specific:
            var_cost = df["var_c_spec_p"].where(
                mask, df["var_c_spec_p"].clip(upper=1)
            )
            fix_cost = df["fix_c_spec_p"].where(
                mask, df["fix_c_spec_p"].clip(upper=1)
            )
    ...
    fig.update_layout(
        height=900, title="Energy System Flows & Costs", hovermode="x unified"
    )
...
def get_flow_cost_breakdown(
    f, all_flow_dict: dict, split: bool = False
) -> pd.DataFrame:
...
def plot_flow_cost_breakdown(
    ...
    for f, rec in all_flow_dict.items():
        ...
        total = pd.DataFrame(
            0.0, index=timeindex, columns=pd.Index([], name="part")
        )
    ...
    def button_args(f, ctype):
        ...
        for c in cols:
            base = (
                c[:-6]
                if c.endswith(" (fix)")
                else (c[:-6] if c.endswith(" (var)") else c)
            )
        ...
        for i, o in enumerate(origins):
            color = {"color": palette[i % len(palette)]}
            col = dfm[o]
        ...
        mode[total_i] = "lines"
        vis[total_i] = True
        line[total_i] = {"color": "black", "width": 2}
    ...
    for s in range(n_slots):
        fig.add_trace(
            go.Scatter(
                ...
                y=first["y"][s],
                name=first["name"][s],
                stackgroup=first["stackgroup"][s],
                mode=first["mode"][s],
                visible=first["visible"][s],
                line=first["line"][s],
...
def print_flow_cost_summary(all_flow_dict):
    ...
    for f, data in all_flow_dict.items():
        ...
        avg_spec_fix = (
            df["fix_c_tot_p"].sum() / df["flow_v"].sum()
            if df["flow_v"].sum() > 0
            else 0
        )
        avg_spec_var = (
            df["var_c_tot_p"].sum() / df["flow_v"].sum()
            if df["flow_v"].sum() > 0
            else 0
        )
        ...
        print(
            f"{label:<40} {avg_spec_fix:>14.4f} {avg_spec_var:>14.4f} {avg_spec_tot:>14.4f} {tot_fix:>12.2f} {tot_var:>12.2f} {tot_total:>12.2f}"
        )
