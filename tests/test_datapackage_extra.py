...
from oemof.eesyplan.datapackage.energy_system import cli
from oemof.eesyplan.datapackage.energy_system import (
    create_energy_system_from_dp,
)
from oemof.eesyplan.datapackage.energy_system import es_to_graphml
from oemof.eesyplan.datapackage.energy_system import plot_es
from oemof.eesyplan.datapackage.energy_system import (
...
def test_es_to_graphml():
    ...
    with tempfile.TemporaryDirectory() as out:
        out_path = Path(out)
        es_to_graphml(es, out_path / "test")
        assert (out_path / "test.graphml").exists()
    ...
    with tempfile.TemporaryDirectory() as out:
        ...
        with warnings.catch_warnings():
            ...
            result = solve_energy_system_from_dp(
                path=DP_JSON, results_path=out_path
            )
...
def test_solve_with_plot_graph():
    with tempfile.TemporaryDirectory() as out:
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=FutureWarning)
            result = solve_energy_system_from_dp(
                path=DP_JSON, plot="graph", results_path=out_path
            )
        assert result is not None
...
def test_plot_es_raises_without_visio():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        with pytest.raises(ModuleNotFoundError, match="oemof-viso"):
            plot_es(es, str(Path(tmp) / "test_path"))
...
def test_solve_with_plot_visio():
    ...
    with mock_renderer as MockRenderer:
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category=FutureWarning)
            result = solve_energy_system_from_dp(path=DP_JSON, plot="visio")
        assert result is not None
